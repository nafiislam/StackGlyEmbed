...
def find_metrics(model_name, y_test):
    if model_name == 'RF':
        ...
        model = LogisticRegression(class_weight='balanced', random_state=1, max_iter=2000)
    elif model_name == 'SVM':
...
features = {
    ...
    'MIF': '/media/nafiislam/New Volume/For side thesis/all_features/MIF/fasta_train_NGlycositeAtlas.csv',
}

with open('./results.csv', 'w') as f1:
    csvWriter = csv.writer(f1)

    csvWriter.writerow(['Features', 'Sensitivity', 'Specificity', 'Balanced Accuracy', 'Accuracy', 'Precision', 'F1-score', 'MCC', 'AUC', 'auPR'])

    for embedding in features:
        print(embedding)
        random.seed(1)

        file_path_Benchmark_embeddings = features[embedding]
        D_feature = pd.read_csv(file_path_Benchmark_embeddings, header=None)
        feature_X_Benchmark_embeddings = D_feature.iloc[:, 2:].values
