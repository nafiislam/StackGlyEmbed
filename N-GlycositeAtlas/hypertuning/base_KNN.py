...
def find_metrics(model_name, y_test, n_neighbors, weights, algorithm, leaf_size, p):
    if model_name == 'KNN':
...
with open("./results_base_KNN.csv", "w") as f:
    writer = csv.writer(f)
    writer.writerow(["n_neighbors, weights, algorithm, leaf_size, p", "Sensitivity", "Specificity", "Balanced_acc", "Accuracy", "Precision", "F1-score", "MCC", "AUC", "auPR"])
    for n_neighbors in grid['n_neighbors']:
        for weights in grid['weights']:
            for algorithm in grid['algorithm']:
                for leaf_size in grid['leaf_size']:
                    for p in grid['p']:
                        random.seed(1)
                        ...
                        for train_index, test_index in cv.split(X, y):
                            ...
                            sensitivity, specificity, bal_acc, acc, prec, f1_score_1, mcc, auc, auPR = find_metrics('KNN', y_test, n_neighbors, weights, algorithm, leaf_size, p)
                        ...
                        print(n_neighbors, weights, algorithm, leaf_size, p)
                        ...
                        writer.writerow([f"'n_neighbors':{n_neighbors}, 'weights':{weights}, 'algorithm':{algorithm}, 'leaf_size': {leaf_size}, 'p': {p}", '{0:.3f}'.format(np.mean(local_Sensitivity)),
