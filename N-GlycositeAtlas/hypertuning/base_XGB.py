...
def find_metrics(model_name, y_test, eta, max_depth, subsample, colsample_bytree, gamma, alpha, reg_lambda):
    if model_name == 'XGB':
        model = XGBClassifier(random_state=1, eta=eta, max_depth=max_depth, subsample=subsample, colsample_bytree=colsample_bytree, gamma=gamma, alpha=alpha,reg_lambda=reg_lambda)
    else:
...
#eta=0.3, max_depth=6, subsample=1, colsample_bytree=1, gamma=0, alpha=0, lambda=0
grid = {
    'eta': [0.001, 0.3],
    'max_depth': [6, 10],
    'subsample': [0.6, 1.0],
...
with open("./results_base_XGB.csv", "w") as f:
    writer = csv.writer(f)
    writer.writerow(["eta, max_depth, subsample, colsample_bytree, gamma, alpha, lambda", "Sensitivity", "Specificity", "Balanced_acc", "Accuracy", "Precision", "F1-score", "MCC", "AUC", "auPR"])
    for eta in grid['eta']:
                        ...
                        for alpha in grid['alpha']:
                            for lambda_ in grid['lambda']:
                                ...
                                for train_index, test_index in cv.split(X, y):
                                    ...
                                    sensitivity, specificity, bal_acc, acc, prec, f1_score_1, mcc, auc, auPR = find_metrics('XGB', y_test, eta, max_depth, subsample, colsample_bytree, gamma, alpha, lambda_)
                                ...
                                print(eta, max_depth, subsample, colsample_bytree, gamma, alpha, lambda_)
                                ...
                                writer.writerow([f"'eta':{eta}, 'max_depth':{max_depth}, 'subsample':{subsample}, 'colsample_bytree': {colsample_bytree}, 'gamma': {gamma}, 'alpha': {alpha}, 'lambda': {lambda_}", '{0:.3f}'.format(np.mean(local_Sensitivity)),
